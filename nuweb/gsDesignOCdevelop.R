...
## test vectors for power
g0 <- gsDesignOC(n.stages=3, rE.seq = c(2,1.5,1), power.efficacy = c(0.5, 0.8, 0.9),
                 futility.type = "none")
g0a <- gsDesignOC(n.stages=3, rE.seq = c(1,1,1), power.efficacy = c(0.5, 0.8, 0.9),
                 futility.type = "none")
# this gives weird result
g0b <- gsDesignOC(n.stages=3, rE.seq = c(1.5,1.5,1), power.efficacy = c(0.5, 0.6, 0.9),
                  futility.type = "none")
g0c <- gsDesignOC(n.stages=2, rE.seq = c(1.5,1), power.efficacy = c(0.5,0.9),
                  futility.type = "none")

g1 <- gsDesignOC(n.stages=3, rE.seq = c(1,1,1), rF.seq=c(0,0,0),
                 power.efficacy = c(0.5, 0.8), power.futility = c(0.5, 0.8),
                 futility.type = "non-binding")
g2 <- gsDesignOC(n.stages=3, rE.seq = c(1,1,1), rF.seq=c(0,0,0),
                 power.efficacy = c(0.5, 0.8), power.futility = c(0.5, 0.8),
